...
function connectRedis(port, ip, password) {
  ...
  */
  this.enqueue = function() {
    if (cluster.isMaster) {
      ...
      this.client.lpush(['jobs', job], (err, reply) => {
        if (err) throw err;
  ...
  */
  this.dequeue = function () {
    if (cluster.isMaster) {
      this.client.rpop('jobs', (err, reply) => {
        if (err) throw err;
...
function replacer(key, value) {
  let omitted = ['cheerio', 'client', 'idle', 'selenium', 'get', 'find', 'retries', 'run', 'setHeaders', 'setInterval', 'setProxies',
   'setWorkers', 'notify', 'store', 'crawler', 'idle', 'run', 'execute', 'executeCheerio', 'executeSelenium', 'giveWorker', 'storeFunction'];
