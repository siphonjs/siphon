...
function processJobs(siphonObj) {
  if (cluster.isMaster) {
    ...
    if (!siphonObj.seleniumOptions && !siphonObj.interval && siphonObj.tries > 1) checkJobs(siphonObj.searchTerms, siphonObj.tries, cluster.workers, statusMessage, siphonObj.html);
...
function checkJobs(searchTerms, tries, workers, statMsg, htmlFunction) {
  ...
  if(remainCount > 0) {
    setTimeout(() => {
      ...
      checkJobs(searchTerms, tries, workers, statMsg, htmlFunction);
