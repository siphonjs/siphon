...
function processJobs(siphonObj) {
  ...
  if (cluster.isMaster) {
    ...
    if (siphonObj.interval) {
      setInterval(() => assignJobs([siphonObj.urls.pop()], siphonObj.searchTerms, siphonObj.tries, cluster.workers, statusMessage, siphonObj.html), siphonObj.interval);
    } else {
      assignJobs(siphonObj.urls, siphonObj.searchTerms, siphonObj.tries, cluster.workers, statusMessage, siphonObj.html);
      siphonObj.urls = [];
    }

    if (!siphonObj.seleniumOptions) checkJobs(siphonObj.searchTerms, siphonObj.tries, cluster.workers, statusMessage, siphonObj.html);
...
*/
function assignJobs(urlArray, searchTerms, tries, workers, statMsg, htmlFunction) {
