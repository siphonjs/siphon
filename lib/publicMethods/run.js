      ...
      setTimeout(() => {
        ...
        if (this.idle) {
          ...
          this.client.rpop('jobsQueue', (err, reply) => {
            ...
            // If response is null, stop operations and reset idle
            try {
...
function createWorker(statMsg, siphonObj) {
  ...
  worker.on('message', (message) => {
    ...
    if (statMsg.jobCount === 0 && siphonObj.idle === false) {
      ...
      console.log(`All jobs completed with ${statMsg.data.length} data points and ${statMsg.errors.length} errors`);
