...
module.exports = function run() {
  if (cluster.isMaster) {
    ...
    if (this.interval) {
      setInterval(() => assignJobs([this.urlArray.pop()], this.tries, cluster.workers, statusMessage), this.interval);
    } else {
      assignJobs(this.urlArray, this.tries, cluster.workers, statusMessage);
      this.urlArray = [];
    }
...
function createWorker(statMsg, siphonObj) {
  ...
  worker.on('message', message => {
    ...
    if (statMsg.jobCount === 0) {
      ...
      console.log(`finished ${statusMessage.data.length} jobs`);
      siphonObj.idle = true;
    }
  });
}
