      ...
      setTimeout( () => {
        ...
        if (this.idle) {
          ...
          this.client.rpop('jobsQueue', (err, reply) => {
            ...
            console.log(typeof this.notifyFunction, typeof this.storeFunction);
...
function processJobs(siphonObj) {
  if (cluster.isMaster) {
    if(siphonObj.searchTerms.length < 1 && !siphonObj.cheerioFunction && !siphonObj.seleniumOptions) return console.error('Please enter a search term, cheerio function, or selenium function and run again');
...
function createWorker(statMsg, siphonObj) {
  ...
  worker.on('message', message => {
    ...
    if (statMsg.jobCount === 0) {
      ...
      if (siphonObj.notifyFunction) siphonObj.notifyFunction(statMsg);
      siphonObj.idle = true;
    }
  });
}
